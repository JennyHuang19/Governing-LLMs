...
1. **Supervised Fine-tuning (SFT):** Human annotators provide “gold standard” responses, and models imitate these responses. *In Anthropic's Constitutional AI method, this step is replaced by sampling from the initial model, then generating self-critiques and revisions according to a principle in the Constitution, then finetune the original model on revised responses.
